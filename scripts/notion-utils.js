...
function checkApiKey() {
  ...
  if (!getApiKey()) {
    const message = 'No routed Notion API token is loaded as NOTION_API_KEY. Resolve the target page or parent with resolve-live-token.js, then use its configured tokenEnv (for example NOTION_API_KEY_WALDEN or NOTION_API_KEY_PERSONAL_CHAD) through the managed route. If the selected token is unavailable, ask which configured Notion workspace to use; never substitute another workspace token.';
    if (hasJsonFlag()) {
